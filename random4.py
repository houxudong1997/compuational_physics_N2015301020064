# -*- coding: utf-8 -*-
"""
Created on Sat Dec 23 15:42:49 2017
...
class random_walk:
    ...
    def plot(self):
        ...
        poly = np.poly1d(para)
        y_fit = poly(self.t)
        pl.plot(self.t, y_fit, 'y', label = 'fit line')
        pl.scatter(self.t, self.r_2ave,color='k',s=2)
        pl.xlim(0,100)
        pl.ylim(0,50)
        pl.xlabel('step number(=time)')
        pl.ylabel('$<r^2>$')
        pl.title('Random walk: 3D(Random length)')
        pl.show()
...
a.set_ylabel('y')
a.set_zlabel('z')
a.set_title('Random walk: 3D(Random length)')
pl.show()